...
/********** Step 5, Offset 2^0 (0*32 + 0) **********/
  ...
  MODBASECASE_QI_ERROR(1<<20, 5, qi, 8);
    ...
    // qi is allways a little bit too small, this is OK for all steps except the last one.
    // Sometimes the result is a little bit bigger than n
    /*
    if(cmp_ge_96(*res,n)) {
        sub_96(&tmp96,*res,n);
        copy_96(res,tmp96);
...
// computes 2^exp mod f, shiftcount is used for precomputing without mod a is precomputed on host ONCE.
{
    int96 f;
